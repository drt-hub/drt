...
- **Performance** — benchmark suite (**#280 — done**, 2026-08-21: fixed 100 / 10,000 / 100,000-row local SQLite → real engine → file-destination scenarios, versioned JSON results with wall time / throughput / peak Python memory / destination-call count, and an importable unmeasured seam for the next profiler) + I/O vs CPU profiling (**#301 — done**, 2026-08-22: cProfile on those unchanged workloads found transformation/serialization CPU at 64.45% / 57.95% / 58.08% versus 35.55% / 42.05% / 41.92% combined I/O; large-run `json.dumps` alone was 39.00%, so [ADR 0010](docs/adr/0010-rust-migration-decision.md) recommends against treating this as evidence for a broad `engine/sync.py` rewrite and defers the final Rust/roadmap call)
