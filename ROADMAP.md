...
- **#919 `GCSWatermarkStorage` race (bug)** — live data-loss bug in shipped code, not a feature; flagged as a v0.8.6 patch candidate (same pattern as #908 → v0.8.5) rather than waiting for a September feature release. **Fixed 2026-08-08** directly on `main` alongside the #756 stack rather than a separate patch release, since v0.9.0 hadn't shipped yet — same failure class caught again as #955 (DLQ, not watermarks), fixed 2026-08-11.
