...
| **v0.7.6** ✅ | **Small follow-up** — Amplitude destination (#574) for user properties (Identify API) + events (HTTP V2 API). New `tojson_safe` Jinja2 filter (#580, PR #581) for REST API `body_template` rendering of `datetime` / `Decimal` / `UUID` columns without `CAST(... AS STRING)` workarounds in model SQL. No breaking changes — drop-in upgrade from v0.7.5. |
