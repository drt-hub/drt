[English](./README.md) | [��ѵ��F�P](./README.ja.md)
...
**drt** syncs data from your data warehouse to external services G�� declaratively, via YAML and CLI.
Think `dbt run` G�� `drt run`. Same developer experience, opposite data direction.
...
No cloud accounts needed G�� runs locally with DuckDB in about 5 minutes.
...
> **Note:** `--show-completion` outputs the script for your *current* shell. Run it from the shell you want to configure. The manual `>>` append is not idempotent G�� run it once only.
...
Install the official Claude Code skills to generate YAML, debug failures, and migrate from other tools G�� all from the chat interface.
...
> `/plugin` G�� Marketplaces G�� drt-hub G�� Enable auto-update
...
|-----------|--------|---------|------|
| BigQuery | G�� v0.1 | `pip install drt-core[bigquery]` | Application Default / Service Account Keyfile |
| DuckDB | G�� v0.1 | (core) | File path |
| PostgreSQL | G�� v0.1 | `pip install drt-core[postgres]` | Password (env var) |
| Snowflake | G�� v0.5 | `pip install drt-core[snowflake]` | Password (env var) |
| SQLite | G�� v0.4.2 | (core) | File path |
| Redshift | G�� v0.3.4 | `pip install drt-core[redshift]` | Password (env var) |
| ClickHouse | G�� v0.4.3 | `pip install drt-core[clickhouse]` | Password (env var) |
| MySQL | G�� v0.5 | `pip install drt-core[mysql]` | Password (env var) |
| Databricks | G�� v0.6 | `pip install drt-core[databricks]` | Access Token (env var) |
| SQL Server | G�� v0.6 | `pip install drt-core[sqlserver]` | Password (env var) |

### Destinations

| Connector | Status | Install | Auth |
|-----------|--------|---------|------|
| REST API | G�� v0.1 | (core) | Bearer / API Key / Basic / OAuth2 |
| Slack Incoming Webhook | G�� v0.1 | (core) | Webhook URL |
| Discord Webhook | G�� v0.4.2 | (core) | Webhook URL |
| GitHub Actions | G�� v0.1 | (core) | Token (env var) |
| HubSpot | G�� v0.1 | (core) | Token (env var) |
| Google Ads | G�� v0.6 | (core) | OAuth2 Client Credentials |
| Google Sheets | G�� v0.4 | `pip install drt-core[sheets]` | Service Account Keyfile |
| PostgreSQL (upsert) | G�� v0.4 | `pip install drt-core[postgres]` | Password (env var) |
| MySQL (upsert) | G�� v0.4 | `pip install drt-core[mysql]` | Password (env var) |
| ClickHouse | G�� v0.5 | `pip install drt-core[clickhouse]` | Password (env var) |
| Parquet file | G�� v0.5 | `pip install drt-core[parquet]` | File path |
| Microsoft Teams Webhook | G�� v0.5 | (core) | Webhook URL |
| CSV / JSON / JSONL file | G�� v0.5 | (core) | File path |
| Jira | G�� v0.5 | (core) | Basic (email + API token) |
| Linear | G�� v0.5 | (core) | API Key (env var) |
| SendGrid | G�� v0.5 | (core) | API Key (env var) |
| Salesforce | =��� v0.6 | `pip install drt-core[salesforce]` | G�� |
| Notion | =��� planned | (core) | G�� |

### Integrations

| Connector | Status | Install |
|-----------|--------|---------|
| Dagster | G�� v0.4 | `pip install dagster-drt` |
| Prefect | G�� v0.6 | (core) |
| Airflow | G�� v0.6 | (core) |
| dbt manifest reader | G�� v0.4 | (core) |

---

## Roadmap

> **Detailed plans & progress G�� [GitHub Milestones](https://github.com/drt-hub/drt/milestones)**
> **Looking to contribute? G�� [Good First Issues](https://github.com/drt-hub/drt/issues?q=is%3Aopen+label%3A%22good+first+issue%22)**

| Version | Focus |
|---------|-------|
| **v0.1** G�� | BigQuery / DuckDB / Postgres sources -+ REST API / Slack / GitHub Actions / HubSpot destinations -+ CLI -+ dry-run |
| **v0.2** G�� | Incremental sync (`cursor_field` watermark) -+ retry config per-sync |
| **v0.3** G�� | MCP Server (`drt mcp run`) -+ AI Skills for Claude Code -+ LLM-readable docs -+ row-level errors -+ security hardening -+ Redshift source |
| **v0.4** G�� | Google Sheets / PostgreSQL / MySQL destinations -+ dagster-drt -+ dbt manifest reader -+ type safety overhaul |
| **v0.5** G�� | Snowflake / MySQL sources -+ ClickHouse / Parquet / Teams / CSV+JSON / Jira / Linear / SendGrid destinations -+ `drt test` -+ `--output json` -+ `--profile` -+ `${VAR}` substitution -+ dbt manifest -+ secrets.toml -+ Docker |
| **v0.5.4** G�� | `destination_lookup` G�� resolve FK values by querying destination DB during sync (MySQL / Postgres / ClickHouse) |
...
<p align="center">
  <img src="docs/assets/ecosystem.png" alt="drt ecosystem G�� dlt load, dbt transform, drt activate" width="700">
</p>
...
We welcome contributions of all sizes G�� from typo fixes to new connectors. drt has a transparent [contributor ladder](GOVERNANCE.md#roles) so your work builds toward greater trust and responsibility over time.

- **Get started:** [CONTRIBUTING.md](CONTRIBUTING.md) G�� setup, workflow, and your first connector tutorial
...
Apache 2.0 G�� see [LICENSE](LICENSE).
