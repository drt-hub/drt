...
- **JSON Schema validation for sync YAML** (#220): Auto-generate JSON Schema from `SyncConfig` Pydantic models via `drt validate --emit-schema`, writing to `.drt/schemas/sync.schema.json`. New `validate_yaml_against_schema()` function for programmatic validation with clear error messages. Enables IDE integration (VS Code YAML extension) for autocomplete and real-time validation.
