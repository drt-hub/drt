...
- **v0.7.7 released** — `sync.mode: mirror` across the SQL destination set. New differential-delete sync mode ([#340](https://github.com/drt-hub/drt/issues/340)) that upserts source rows and DELETEs destination rows whose `upsert_key` was not observed in the source — no TRUNCATE / re-insert overhead of `replace` mode. Lands across **Postgres** (PR [#596](https://github.com/drt-hub/drt/pull/596)), **MySQL** (PR [#597](https://github.com/drt-hub/drt/pull/597)), **ClickHouse** (PR [#598](https://github.com/drt-hub/drt/pull/598) — `ALTER TABLE ... DELETE` mutation w/ `mutations_sync=1`), **Snowflake** (PR [#599](https://github.com/drt-hub/drt/pull/599) — MERGE-path forcing + first-ever `finalize_sync` on Snowflake). BigQuery follows in v0.7.8 when contributor PR [#584](https://github.com/drt-hub/drt/pull/584) lands. Also lands the **`cli/main.py` split completion** — Phase 2b PR (a) + PR (b) + tighten (PRs [#579](https://github.com/drt-hub/drt/pull/579) / [#587](https://github.com/drt-hub/drt/pull/587) / [#591](https://github.com/drt-hub/drt/pull/591)) finish the **1706 → 164 LOC (-90%)** split begun in v0.7.5 — plus `FakeSource` + destination contract test framework (#592–#595), CI `check-changelog-required` warn-only guard (#590), GCS storage import mypy fix (#588), and CI install line extension that unlocked ~102 silently-skipped SQL destination tests (raised total coverage 82.68 → 85.29). No breaking changes — drop-in upgrade from v0.7.6.
