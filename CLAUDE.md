...
- **v0.7.6 released** — Small follow-up. Two additive features: **Amplitude destination** (#574) covering both the Identify API (user properties) and the HTTP V2 API (events), and the **`tojson_safe` Jinja2 filter** (#580 / PR [#581](https://github.com/drt-hub/drt/pull/581)) that unblocks `datetime` / `Decimal` / `UUID` columns flowing into REST API `body_template` rendering without `CAST(... AS STRING)` workarounds in model SQL. No breaking changes — drop-in upgrade from v0.7.5.
