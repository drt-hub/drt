...
def test_validate_check_connection_sql_success() -> None:
    ...
    with (
        patch("drt.connectors.registry.get_destination", return_value=mock_dest),
        ...
        mock_sync.destination = PostgresDestinationConfig(
            type="postgres", table="t", upsert_key=["id"], host="localhost", dbname="db"
        )
...
def test_validate_check_connection_non_sql_gated_type_success() -> None:
    ...
    with (
        ...
        result = runner.invoke(app, ["validate", "--check-connection", "--select", "bigquery_sync"])
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "bigquery_sync",
            BigQueryDestinationConfig(type="bigquery", project="p", dataset="d", table="t"),
...
def test_validate_check_connection_runs_least_privilege_connector_probes(
    ...
    with (
        ...
        result = runner.invoke(app, ["validate", "--check-connection", "--select", name])
...
def test_validate_check_connection_json() -> None:
    ...
    with (
        ...
        assert sync_res["connection_test"] == {"success": True, "error": None, "skipped": False}
...
def test_validate_check_connection_skipped_json() -> None:
    ...
    with patch("drt.config.parser.load_syncs_safe") as mock_load:
        ...
        sync_res = data["results"][0]
        assert sync_res["connection_test"] == {"success": None, "error": None, "skipped": True}
