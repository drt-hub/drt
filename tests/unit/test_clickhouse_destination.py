...
class TestClickHouseDestinationLoad:
    ...
    def test_insert_batches_by_sync_batch_size(self, mock_connect: MagicMock) -> None:
        ...
        result = ClickHouseDestination().load(records, _config(), _options(batch_size=100))
    ...
    @patch("drt.destinations.clickhouse.ClickHouseDestination._connect")
    def test_ambiguous_batch_failure_retries_with_same_token(self, mock_connect: MagicMock) -> None:
        client = _fake_client()
    ...
    def test_row_error_on_error_skip(self, mock_connect: MagicMock) -> None:
        ...
        def insert_with_one_bad_record(_table: str, rows: list[list[Any]], **_kwargs: Any) -> None:
    ...
    def test_row_error_on_error_fail(self, mock_connect: MagicMock) -> None:
        ...
        def fail_during_batch_fallback(_table: str, rows: list[list[Any]], **_kwargs: Any) -> None:
            if len(rows) > 1:
                raise Exception("batch rejected")
            if rows[0][0] == 2:
