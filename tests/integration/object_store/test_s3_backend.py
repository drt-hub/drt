"""S3 backend conformance and concurrency tests against LocalStack."""
...
def test_00_s3_precondition_canary(s3_preconditions_supported: bool) -> None:
    assert s3_preconditions_supported


def test_state_store_conforms_for_read_write_overwrite_and_reset(
    s3_client: ObjectClient,
) -> None:
    store = ObjectStoreStateStore(s3_client, prefix=_prefix())
...
def test_history_store_conforms_for_merge_limit_and_prune(
    ...
    store = ObjectStoreHistoryStore(s3_client, prefix=_prefix(), max_entries=2)
...
def test_dlq_store_conforms_for_fifo_replace_clear_and_depths(
    ...
    prefix = _prefix()
    store = ObjectStoreDlqBackend(s3_client, prefix=prefix)
...
def test_concurrent_state_writers_never_silently_lose_updates(
    ...
    workers = 8
    counting = CountingClient(s3_client)
    prefix = _prefix()
