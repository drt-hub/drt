...
| **v0.7.6** ✅ | **小規模フォローアップ** — Amplitude destination (#574) でユーザープロパティ (Identify API) とイベント (HTTP V2 API) の双方に対応。`tojson_safe` Jinja2 フィルタ (#580, PR #581) を新設し、REST API `body_template` で `datetime` / `Decimal` / `UUID` カラムを model SQL 側で `CAST(... AS STRING)` せずに送れるように。破壊変更なし — v0.7.5 からドロップイン更新可。 |
