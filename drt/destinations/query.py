...
def fetch_rows(
    ...
    ``field_hint`` (optional, only consulted when ``columns`` is empty) is
    the caller's own expected column-name casing — the *full* source record
    field set, not just ``upsert_key`` (#1064: a non-key column can suffer
    the identical collapse if its casing goes unreconciled too) — used by
    dialects whose driver doesn't round-trip a column's configured case
    (Snowflake folds unquoted identifiers to uppercase; live-verified MySQL
    does not, #1064, so it passes ``columns`` explicitly instead and never
    reaches this path). Reconciled case-insensitively per column rather than
    a blanket transform in either direction, so a sync genuinely configured
    with uppercase field names (e.g. ``upsert_key: ["ID"]``, itself
    Snowflake-native) is not corrupted just as much as an unreconciled
    uppercase would corrupt the common lowercase case (#1062-followup,
    caught in review across two passes).
    """
    if isinstance(config, PostgresDestinationConfig):
    ...
    if isinstance(config, SnowflakeDestinationConfig):
        return _fetch_rows_snowflake(config, query, columns, field_hint)
    if isinstance(config, DatabricksDestinationConfig):
        return _fetch_rows_databricks(config, query, columns)
    raise TypeError(f"Cannot fetch rows from {type(config).__name__}")


def _reconcile_column_case(metadata_names: list[str], field_hint: list[str] | None) -> list[str]:
    """Map ``metadata_names`` (e.g. Snowflake's uppercase-folded columns)
    onto ``field_hint``'s casing wherever a case-insensitive match exists,
    falling back to lowercase for the rest (the common, non-hinted case)."""
    hint_by_lower = {c.lower(): c for c in field_hint} if field_hint else {}
...
def _fetch_rows_snowflake(
    ...
    review, #1062-followup, then refined across a second pass, then widened
    to non-key fields per #1064): Snowflake's cursor metadata reports
    unquoted columns in their stored UPPERCASE form, but source record
    fields are conventionally lowercase — unreconciled, ``compute_diff()``'s
    key lookups and its ``changed_fields()`` comparison would both see
    ``None`` for every column and either collapse rows into one keyless
    entry (key columns) or misreport every row as changed (non-key columns).
    A blanket lowercase fold fixes the common case but corrupts the opposite
    one (a sync genuinely configured with uppercase field names, itself
    Snowflake-native) — ``field_hint`` (the caller's full set of known
    source record field names, not just ``upsert_key``) lets
    ``_reconcile_column_case`` match case-insensitively and prefer the
    caller's actual casing per column, falling back to lowercase only for
    columns it has no hint for.
    """
    from drt.destinations.snowflake import SnowflakeDestination
