...
<meta name="description" content="{{ project_name }} — sync catalog & lineage, generated by drt">
<link rel="icon" href="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 32 32'%3E%3Crect width='32' height='32' rx='7' fill='%237c3aed'/%3E%3Ctext x='16' y='23' font-family='sans-serif' font-size='19' font-weight='700' fill='white' text-anchor='middle'%3Ed%3C/text%3E%3C/svg%3E">
{% if inline_assets %}<style>{{ inline_style|safe }}</style>
<style>{{ inline_pygments|safe }}</style>{% else %}<link rel="stylesheet" href="{{ root }}assets/style.css">
<link rel="stylesheet" href="{{ root }}assets/pygments-default.css">{% endif %}
{% block head %}{% endblock %}
</head>
<body>
...
{% if inline_assets %}<script>{{ inline_app_js|safe }}</script>{% else %}<script src="{{ root }}assets/app.js"></script>{% endif %}
...
<div class="tabs" role="tablist">
  <button class="tab-btn active" data-tab="yaml" role="tab" aria-selected="true">YAML</button>
  <button class="tab-btn" data-tab="lineage" role="tab" aria-selected="false">Lineage</button>
  <button class="tab-btn" data-tab="state" role="tab" aria-selected="false">State</button>
</div>
...
def render_html(
    ...
    With *inline_assets* (CLI: ``--inline``, #818) each page embeds the CSS and
    JS inline (`<link>`/`<script src>` → `<style>`/`<script>`) and the shared
    ``assets/`` directory is not written, so every page renders with **zero
    sub-resource requests**. This is what makes the catalog viewable on an
    authenticated object store (GCS ``storage.cloud.google.com`` links, S3
    presigned URLs) where per-object auth 401s the relative ``assets/*``
    fetches. Output stays multi-page — inter-page links still work because each
    target page is itself self-contained. The default (multi-file) output is
    unchanged.
    ...
    # Asset payloads, shared by the default asset files and --inline embedding
    # (#818). Deterministic (constants + pygments style defs), so inline output
    # stays byte-identical across regenerations like the default (#697).
    pygments_css = (
    ...
    common = {
        ...
        "inline_pygments": pygments_css if inline_assets else "",
        "inline_app_js": APP_JS if inline_assets else "",
    }
    ...
    # assets/ — skipped in --inline mode, where each page carries CSS/JS itself
    # (#818), so the site has no external sub-resources to 401 on object stores.
    if not inline_assets:
        write("assets/style.css", STYLE_CSS)
        write("assets/app.js", APP_JS)
        write("assets/pygments-default.css", pygments_css)
