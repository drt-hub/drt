...
Tracked mirror uses a temporary staging table for a server-side diff when the role has schema-level `CREATE TABLE`; that privilege remains optional. A role limited to the target and pre-provisioned state tables transparently falls back to a client-side diff, preserving the no-DDL deployment pattern. The fallback reads that sync's tracked-key state into process memory, so very large state tables trade the server-side optimisation for higher client memory use.
