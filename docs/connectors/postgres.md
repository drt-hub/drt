...
Choose `destination` when drt owns the table (slightly cheaper: no state I/O). Choose `tracked` when anything else writes to the table. Currently supported on **Postgres, MySQL, Snowflake, and ClickHouse** ([#692](https://github.com/drt-hub/drt/issues/692)); Databricks rejects `strategy: tracked` with a clear error until its follow-up lands.
