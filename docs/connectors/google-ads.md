...
- Without `cloud_project_id`, every `google_ads` config sharing one `drt run` process shares a single, conservative rate-limit bucket regardless of token or OAuth client — safe (never causes a 429), but coarser than necessary if you run several genuinely independent Cloud projects' syncs concurrently. Set `cloud_project_id` per config to split them correctly.
