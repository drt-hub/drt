<!-- i18n-sync: base=CONTRIBUTING.md, hash=eb6b6afacff97458ef6118bd2b229794f6e8d83e -->
...
- [ ] テストがパスする（`make test`）
- [ ] リンターがパスする（`make lint`）
- [ ] `CHANGELOG.md` の `[Unreleased]` セクションが更新されている — pure な CI / 依存バンプ / i18n marker 以外は全て対象（ユーザー向け変更・内部リファクタ・バグ修正を含む）。`make check-changelog-required` で CI 警告として可視化されます。バイパス（CI/依存バンプ・i18n marker のみ）は PR 説明に `[skip changelog]` を含める。
...
### CI ガード: リリース済みバージョンの CHANGELOG セクションを失わないこと

`vX.Y.Z` の git タグはすべて `CHANGELOG.md` に対応する `## [X.Y.Z]` セクションを
持っていなければなりません。CI は `make check-changelog` でこれを enforce します
— PR がこれで失敗した場合の最も多い原因は、main の状態にファイルを「リストア」する
タイプのマージコンフリクト解決で、リリース済みバージョンのセクションを誤って削除
してしまうケースです。失われたセクションをタグから復元するには:

```bash
git show v0.7.4:CHANGELOG.md | sed -n '/^## \[0.7.4\]/,/^## \[/p'
```

…結果を `CHANGELOG.md` の正しい位置（`## [Unreleased]` の後、次に古いバージョン
の前）に貼り付け、`make check-changelog` を再実行してください。

既存のリリース済みバージョンセクションの内容を編集すること（タイポ修正、リンク
修復など）は問題ありません — ガードはセクション見出しの存在のみ検査し、内容は
検査しません。
